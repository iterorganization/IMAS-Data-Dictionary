...
def main():
    ...
    try:
        ...
        try:
            ...
            doc_path = package_files / "resources" / "docs" / "legacy" / "html_documentation.html"
            doc_file = str(doc_path)
        except (ImportError, AttributeError):
            # Fallback for Python 3.8
            import importlib.resources as resources
            with resources.path("imas_data_dictionary", "resources"):
                resource_path = Path(resources.__file__).parent / "imas_data_dictionary" / "resources"
            doc_file = str(resource_path / "docs" / "legacy" / "html_documentation.html")
        
        # Alternative: direct path lookup
        if not Path(doc_file).exists():
            # Try to find it relative to this package
            package_dir = Path(__file__).parent
            doc_file = str(package_dir / "resources" / "docs" / "legacy" / "html_documentation.html")
        
        if not Path(doc_file).exists():
            print("[IMAS-DD] Error: Documentation file not found", file=sys.stderr)
            print(f"[IMAS-DD] Searched locations:", file=sys.stderr)
            print(f"  - {doc_file}", file=sys.stderr)
            print("[IMAS-DD] Documentation may not have been installed. Install with:", file=sys.stderr)
        ...
        if success:
            ...
            print("[IMAS-DD] Warning: Could not open browser, please open manually:", file=sys.stderr)
